<x-admin-layout>
    <div class="space-y-6">
        ...
        <div class="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-yellow-400 via-purple-500 to-blue-600 p-6 sm:p-8 text-white shadow-2xl shadow-purple-200">
            ...
            <p class="mt-2 max-w-xl text-white/90">
                Perbarui jadwal, lokasi, waktu, dan status agenda kegiatan.
            </p>
        </div>

        <form action="{{ route('admin.agendas.update', $agenda) }}"
              method="POST"
              class="rounded-[2rem] bg-white p-5 sm:p-8 shadow-xl shadow-slate-200/70 space-y-6">
            @csrf
            @method('PUT')

            @include('admin.agendas.form', ['agenda' => $agenda])

            <div class="flex flex-col sm:flex-row gap-3 pt-4">
                <button type="submit"
                        class="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-purple-600 to-blue-600 px-5 py-4 font-black text-white shadow-xl shadow-purple-200">
                    💾 Update Agenda
                </button>
