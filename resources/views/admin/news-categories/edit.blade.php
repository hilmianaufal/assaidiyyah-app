<x-admin-layout>
    <div class="space-y-6">
        <div class="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-yellow-400 via-blue-600 to-purple-600 p-6 sm:p-8 text-white shadow-2xl shadow-blue-200">
            ...
            <p class="mt-2 max-w-xl text-white/90">
                Perbarui nama, warna, dan deskripsi kategori.
            </p>
        </div>

        <form action="{{ route('admin.news-categories.update', $category) }}"
              method="POST"
              class="rounded-[2rem] bg-white p-5 sm:p-8 shadow-xl shadow-slate-200/70 space-y-6">
            @csrf
            @method('PUT')

            @include('admin.news-categories.form', ['category' => $category])

            <div class="flex flex-col sm:flex-row gap-3 pt-4">
                <button type="submit"
                        class="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 px-5 py-4 font-black text-white shadow-xl shadow-blue-200">
                    💾 Update Kategori
                </button>
