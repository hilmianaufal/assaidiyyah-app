<x-admin-layout>
    <div class="space-y-6">
        <div class="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-slate-900 via-indigo-700 to-blue-600 p-6 sm:p-8 text-white shadow-2xl shadow-blue-200">
            <div class="absolute right-6 bottom-4 text-7xl opacity-20">#️⃣</div>
            ...
            <p class="mt-2 max-w-xl text-white/90">
                Tambahkan tag untuk menandai topik berita.
            </p>
        </div>

        <form action="{{ route('admin.news-tags.store') }}"
              method="POST"
              class="rounded-[2rem] bg-white p-5 sm:p-8 shadow-xl shadow-slate-200/70 space-y-6">
            @csrf

            @include('admin.news-tags.form', ['tag' => null])
